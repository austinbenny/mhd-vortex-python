...
\begin{frame}{Why MHD}
  \begin{itemize}
    \item A conducting fluid in motion is a distributed electric
          current; the \emph{Lorentz force} $\mathbf{J}\times\mathbf{B}$
          is a body force on every cell~\cite{chen2016,freidberg2014}.
    \item ``Frozen-in flux'': in the ideal limit, field lines move
          with the fluid. Fluid and field are tied together everywhere.
    \item Valid when plasma is collisional and magnetic Reynolds
          number is large: solar wind, tokamaks, astrophysical jets,
          magnetospheres.
    \item New physics vs Euler:
      \begin{itemize}\small
        \item \emph{Alfven} waves---transverse, like waves on a
              string with magnetic tension as the restoring force.
        \item \emph{Fast/slow magnetosonic} waves---sound waves
              modified by magnetic pressure.
...
\begin{frame}{Ideal MHD in conservative form}
  ...
  \begin{itemize}
    ...
    \item New vs Euler: \emph{magnetic pressure} $\tfrac{1}{2}|B|^{2}$
          inside $p_T$, \emph{magnetic tension} along field lines via
          $-B_iB_j$, and \emph{magnetic energy} $\tfrac{1}{2}|B|^{2}$
          in $E$.
    \item Induction-equation flux $u B_y - v B_x$ is the $z$-component
          of the motional electric field $-\mathbf{v}\times\mathbf{B}$:
          this is what drags field lines along with the flow.
...
\begin{frame}{The divergence constraint}
  \begin{itemize}
    \item $\nabla\cdot B = 0$ is Gauss's law for magnetism: no magnetic
          monopoles, so field lines have no sources or sinks.
    \item Exact ideal MHD preserves this analytically; piecewise-constant
          Godunov updates do not---reconstructed interface fields produce
          a small but nonzero divergence at every cell
          center~\cite{balsara1999,powell1999}.
    \item Consequence is direct: rewriting
          $(\nabla\times B)\times B = (B\cdot\nabla)B - \tfrac{1}{2}\nabla|B|^{2}
          - (\nabla\cdot B)B$, the last term becomes a spurious
          force \emph{parallel to $B$}, breaks momentum conservation,
          and eventually blows the run up.
